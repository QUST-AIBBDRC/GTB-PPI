...
from dimensionality_reduction_com import factorAnalysis
...
label=data[:,0]
data_2=factorAnalysis(shu,percentage=0.90)
shu=data_2
X=shu[index,:]
y=label[index]
sepscores = []
...
yscore_sum.to_csv('yeast_yscore_sum_FA_90.csv')# save the data
ytest=ytest[np.array(range(1,row)),:]
ytest_sum = pd.DataFrame(data=ytest)
ytest_sum.to_csv('yeast_ytest_sum_FA_90.csv')
...
data_csv.to_csv('yeast_result_FA_90.csv')
